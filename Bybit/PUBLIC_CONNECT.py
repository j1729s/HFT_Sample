...
class BybitOBIngest:
        ...
        while True:
            try:
                ...
                async with websockets.connect(
                    ...
                    try:
                        async for raw in ws:
                            # local receive time in ms (keep this)
                ...
                backoff = 0.25  # reset after a clean session

            except asyncio.CancelledError:
                self.log.exception(f"conn={conn_id} ws_error err=CANCELLED")
                self.log.info("Restarting!!")
                await asyncio.sleep(backoff*4)
            except Exception as e:
    ...
    def _accept(self, msg: dict) -> bool:
        k = extract_key(msg)
        if k is None:
            # log + drop: if we can't key it, don't let it into "fastest-first" stream
    ...
    async def consume_and_update_and_print_state(self):
        while True:
            ...
            while self.q:
                ...
                bb, ba = self.state.get_ticker()
                #bids, asks = self.state.get_ob()
